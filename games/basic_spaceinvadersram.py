...
class MuZeroConfig:
    def __init__(self):
        ...
        self.action_space = [i for i in range(6)]  # Fixed list of all possible actions. You should only edit the length
        ...
        self.temperature_threshold = 0  # Number of moves before dropping temperature to 0 (ie playing according to the max)
        ...
        self.fc_reward_layers = [32, 32, 8, 8]  # Define the hidden layers in the reward network
        self.fc_value_layers = [32, 32, 8, 8]  # Define the hidden layers in the value network
        self.fc_policy_layers = [32, 32, 8, 8]  # Define the hidden layers in the policy network
        self.fc_representation_layers = [128, 64, 64]  # Define the hidden layers in the representation network
        self.fc_dynamics_layers = [32, 32, 8, 8]  # Define the hidden layers in the dynamics network
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = gym.make("SpaceInvaders-ram-v4")
        if seed is not None:
            self.env.seed(seed)
